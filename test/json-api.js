  ...
  _.each(['memory'], function(dbType) {
    ...
    describe('should work in ' + dbType + ' storage', function() {
      ...
      it('GET /api/revisions/2/captures?status=SUSPICIOUS should filter docs by status in' + dbType, function(done) {
        ...
        .expect({
          revision: { id: '2', updatedAt: '1970-01-01T00:00:00.000Z' },
          captures:
           [{ id: 'revision:2:capture:db38f7f3f5d7d765f97e45d185066cc9',
               revision: '2',
               capture: 'db38f7f3f5d7d765f97e45d185066cc9',
               captureName: 'scenario2/capture2.png',
               modifiedStatus: 'IS_OK',
               updatedAt: '1970-01-01T00:00:00.000Z',
               status: 'SUSPICIOUS',
               span: 10,
               threshold: 5,
      ...
      it('GET /api/revisions/2/captures?modifiedStatus=UNPROCESSED should filter docs by modifiedStatus in' + dbType, function(done) {
        request(app)
        .get('/api/revisions/2/captures?modifiedStatus=UNPROCESSED')
        .expect({
          revision: { id: '2', updatedAt: '1970-01-01T00:00:00.000Z' },
          captures:
           [{ id: 'revision:2:capture:9018988ae55e012e437aa24cbf9a400a',
